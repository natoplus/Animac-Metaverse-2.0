...
async def list_articles(category: Optional[str] = None, featured: Optional[bool] = None, is_published: bool = True, limit: int = 10, skip: int = 0):
...
async def get_article_by_id(article_id: str):
    res = supabase.table("articles").select("*").eq("id", article_id).limit(1).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Article not found")
    return ArticleResponse(**res.data[0])
...
async def search_articles(params: SearchParams):
    ...
    if params.query:
        q = q.ilike("title", f"%{params.query}%")
    if params.category:
...
async def toggle_article_like(article_id: str, request: Request):
...
async def toggle_article_bookmark(article_id: str, request: Request):
    session_id = request.headers.get("X-Session-ID") or str(uuid.uuid4())
...
@app.get("/api/articles/{article_id}/is-liked")
async def is_article_liked(article_id: str, request: Request):
    session_id = request.headers.get("X-Session-ID")
    if not session_id:
        return {"liked": False}
    like_check = supabase.table("article_likes").select("id").eq("article_id", article_id).eq("session_id", session_id).execute()
    return {"liked": bool(like_check.data)}

@app.get("/api/articles/{article_id}/is-bookmarked")
async def is_article_bookmarked(article_id: str, request: Request):
    session_id = request.headers.get("X-Session-ID")
    if not session_id:
        return {"bookmarked": False}
    bookmark_check = supabase.table("article_bookmarks").select("id").eq("article_id", article_id).eq("session_id", session_id).execute()
    return {"bookmarked": bool(bookmark_check.data)}
