...
async function fetchAniListTrending() {
  try {
    ...
    console.warn('[WatchTower] AniList trending failed, falling back to Jikan:', error.message);
    return fetchJikanTrending();
  }
}
async function fetchAniListTop() {
  try {
    ...
    console.warn('[WatchTower] AniList top failed, falling back to Jikan:', error.message);
    return fetchJikanTop();
  }
}
async function fetchAniListUpcoming() {
  try {
    ...
    console.warn('[WatchTower] AniList upcoming failed, falling back to Jikan:', error.message);
    return fetchJikanUpcoming();
  }
}
...
function useWatchTowerData(mode /* 'east' | 'west' */) {
  ...
  const [apiStatus, setApiStatus] = useState({ anilist: true, jikan: true });
...
export default function WatchTowerPage() {
  ...
  return (
    <>
      ...
      <div className="bg-yellow-600/20 border-l-4 border-yellow-500 text-yellow-100 px-4 py-3 mx-4 mt-4 rounded">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <p className="text-sm">
              <strong>Service Notice:</strong> AniList API is temporarily down. We're using alternative data sources to keep your experience smooth.
            </p>
          </div>
        </div>
      </div>
