      ...
      <div className={`relative overflow-hidden bg-gradient-to-br ${theme.gradient}`}>
        ...
        <div className="relative z-20 container mx-auto px-4 py-16 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-4">
            <button onClick={() => setLiked(!liked)} className={`px-4 py-2 rounded-lg flex items-center gap-2 border transition ${liked ? `${theme.button} ${theme.border} text-white` : 'bg-transparent border-gray-600 text-gray-300 hover:border-gray-400 hover:text-white'}`}>
              <Heart size={18} fill={liked ? 'currentColor' : 'none'} />
              Like
            </button>

            <button onClick={() => setBookmarked(!bookmarked)} className={`px-4 py-2 rounded-lg flex items-center gap-2 border transition ${bookmarked ? `${theme.button} ${theme.border} text-white` : 'bg-transparent border-gray-600 text-gray-300 hover:border-gray-400 hover:text-white'}`}>
